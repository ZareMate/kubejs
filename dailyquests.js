...
function saveData() {
	...
	try {
		...
		console.error("[DailyQuests] Failed to save data: " + String(error));
	}
}
...
function countItem(player, itemId) {
	...
	try {
		...
		console.error("[DailyQuests] Inventory count error: " + String(error));
...
function tellQuest(player, category, quest, claimed) {
	...
	var itemName =
		quest.itemName || quest.description.replace(/^Collect\s+\d+\s+/i, "");
...
function claimQuest(player, category) {
	...
	if (!reward) {
		player.tell(
			Text.of("This quest has an invalid reward configuration.").red(),
		);
	...
	if (currentAmount < quest.amount) {
		...
		player.tell(
			Text.of(
				"Required: " + quest.amount + " | You have: " + currentAmount,
			).gray(),
		);
	...
	if (countItem(player, quest.item) < quest.amount) {
		player.tell(
			Text.of("Your inventory changed. The claim was cancelled.").red(),
		);
...
function rerollPlayer(server, name) {
	...
	data.players[savedPlayer.uuid] = createOfflinePlayerData(savedPlayer.name);

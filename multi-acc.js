...
(function () {
	...
	function getLuckPerms() {
		...
		try {
			...
			console.log(
				"[Multi-Account] luckPermsApi API loaded successfully!",
			);
	...
	function sendCheckIPWebhook(player, groups) {
		...
		try {
			...
			console.error(
				"[SputnikWatch] Failed to send /checkip webhook: " + e,
			);
		}
	}
			...
			Commands.literal("checkip")
				...
				.executes((ctx) => {
					...
					for (var players of playerIPMap.values()) {
						...
						source.sendSuccess(
							Text.of(
								`§7Group ${group}: §e${playerList.join(", ")}`,
							),
							false,
						);
	...
	console.log(
		"[Multi-Account] /checkip permission: " +
			MULTIACCOUNT_PERMISSION_CHECKIP,
	);

	console.log(
		"[Multi-Account] Chat alert permission: " +
			MULTIACCOUNT_PERMISSION_ALERT,
	);
